...
class IconAnimator {
	...
	/// The icons list where we perfrom animation
	private var iconsList:[String] = []
	...
	func setDefaultIcon(icon:String){
		appProxy.setAlternateIconName(icon) { success, error in
	...
	private func updateFrame() -> Bool {
		...
		let iconName = iconsList[currentFrame]
		...
		appProxy.setAlternateIconName(iconName) { success, error in
			if !success || error != nil {
				print("Error:\(iconName)----- \(error as Any)")
				return
			}
		}
...
class IconAnimatorHandler {
	...
	static let shared = IconAnimatorHandler()
	
	@AppStorage("appdefaultlogo") var setting_active_app_icon:appIcon = .def
	
	func createAnimator() -> IconAnimator{
		let appicons = ["AppIcon0","AppIcon1","AppIcon2","AppIcon3","AppIcon4","AppIcon5","AppIcon6","AppIcon7"]
	
		return IconAnimator(
			numberOfFrames: appicons.count,
			numberOfLoops: appicons.count,
			iconsList:  appicons,
			targetFramesPerSecond: CGFloat(appicons.count),
	...
	func startAnimation() {
		...
		animator.startAnimation() { [weak self] in
			// Once the animation is complete,
			// end our background task so that iOS knows
			// that our app has finished its work
			guard let self = self else { return }
			
			animator.setDefaultIcon(icon: setting_active_app_icon.rawValue)
